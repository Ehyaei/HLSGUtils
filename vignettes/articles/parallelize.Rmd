...
Suppose we want to run different linear models on a dataset. We can write it with a for loop in R, but as a result of many R functions working with one core, this process, It will eventually run on one core and is time-consuming. Another solution is to use parallel packages in R, like parallel. This package is very good, but the time of the executions does not decrease linearly when we increase the computation cores. One solution is that we run our code in parallel in different R sessions manually, like the Job option in Rstudio. At first glance, this idea is hard to implement because we need to have different R scripts and run them simultaneously. The `HLSGUtils` package provides some functions to make this work simpler with dynamic system resource management of memory and thread options.
...
`function_to_Rscript` convert function to command line format. This function needs:

- `function_from_source`: The path of the saved R function.
- `function_name`: Name of the function called in the R script.
- `packages`: The packages that are needed to to call.
- `arguments`: Names of function arguments.
- `arguments_class`: Classes Type of function arguments.
...
Finally, we want to run multiple models in parallel. `parallel_rscripts` allows you to run R command line functions in parallel.It needs to set input variables and system resource management thresholds.

- `rscript_path`: The path of command line format script
- `args`: list of function arguments
- `used_memory_treshold`: The total percentage of the system's used memory for computation.
- `used_cpu_treshold`: The total percentage of the thread's used memory for computation.
